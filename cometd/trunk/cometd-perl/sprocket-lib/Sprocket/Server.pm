...
sub local_accept {
    ...
    $self->process_plugins( [ 'local_accept', $con, $socket ] );
...
sub accept {
    ...
    $con->wheel_readwrite(
        ...
        ErrorEvent      => $con->event( 'local_rw_error' ),
    ...
    $self->process_plugins( [ 'local_connected', $con, $socket ] );
...
sub local_receive {
    ...
    $self->process_plugins( [ 'local_receive', $con, $_[ARG0] ] );
...
sub local_flushed {
    ...
    $self->process_plugins( [ 'local_flushed', $con ] )
        if ( $con->watch_flush );
...
sub local_wheel_error {
    ...
    $self->process_plugins( [ 'local_error', $operation, $errnum, $errstr ] );
...
sub local_rw_error {
    ...
    $self->process_plugins( [ 'local_disconnected', $con, 1, $operation, $errnum, $errstr ] );
...
sub begin_soft_shutdown {
    ...
    foreach ( values %{$self->{heaps}} ) {
        next unless defined;
        $self->process_plugins( [ 'local_shutdown', $_ ] );
    }
