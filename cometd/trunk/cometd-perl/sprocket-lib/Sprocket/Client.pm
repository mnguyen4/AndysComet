...
sub remote_connect_success {
    ...
    $con->socket( $socket );
    $self->process_plugins( [ 'remote_accept', $con, $socket ] );
...
sub accept {
    ...
    $self->process_plugins( [ 'remote_connected', $con, $socket ] );
...
#    if ( $con->connected ) {
        $self->process_plugins( [ 'remote_disconnected', $con, @_[ ARG0 .. ARG2 ] ] );
#    } else {
#        $self->process_plugins( [ 'remote_connect_error', $con ] );
#    }
...
sub remote_connect_timeout {
    ...
    $self->process_plugins( [ 'remote_connect_error', $_[ HEAP ] ] );
...
sub remote_receive {
    ...
    $self->process_plugins( [ 'remote_receive', @_[ HEAP, ARG0 ] ] );
...
sub remote_error {
    ...
    $self->process_plugins( [ 'remote_disconnected', $con, 1, $operation, $errnum, $errstr ] );
...
sub connect {
    ...
    ( $address, $port ) = @$address if ( isa_ARRAY( $address ) );

    return $self->_log( v => 1, msg => 'Host/port not defined in call to connect, IGNORED.' )
...
sub resolved_address {
    ...
    unless ( defined $response_obj ) {
        ...
        $self->process_plugins( [ 'remote_connect_error', $con, $response_err, $response_obj ] );
        return;
    }
...
sub begin_soft_shutdown {
    ...
    foreach ( values %{$self->{heaps}} ) {
        next unless defined;
        $self->process_plugins( [ 'remote_shutdown', $_ ] );
    }
}
