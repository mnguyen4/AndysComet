...
#        $kernel->alias_remove($self->{alias}) if ($self->{alias} ne '');
#        # and the child
#        $kernel->sig( 'CHLD' );
    }
...
sub combo_query {
    ...
    my $handle = sub {
        push(@res, shift);
        $poe_kernel->post( $args->{session} => $args->{event} => @res ) if (delete $res[ -1 ]->{__edbi_last});
    };
    ...
    foreach my $i ( 0 .. $#{ $args->{queries} } ) {
        ...
        foreach (
            qw( id sql placeholders no_cache begin_work commit )
            ,@{$self->{action_params}->{$type}}
        ) {
            next unless (defined($args->{$_}));
            $arg->{$_} = $args->{$_};
        }
        $arg->{event} = $handle;
        $arg->{__edbi_last} = ( $i == $#{ $args->{queries} } );
...
sub check_queue {
    ...
    return unless (scalar(@{ $self->{queue} }) > 0);
    
    # shutting down?
    return unless ($self->{shutdown} != 2);
    
    if ($self->{opts}{stopwatch}) {
        tie $self->{queue}->[0]->{stopwatch}, 'Time::Stopwatch';
    }
    ...
    foreach (
        ...
        ,@{$self->{action_params}->{$self->{queue}->[0]->{action}}}
    ) {
        next unless (defined($self->{queue}->[0]->{$_}));
        $queue{$_} = $self->{queue}->[0]->{$_};
    }
...
sub start {
    ...
    $kernel->alias_set($self->{alias}) if ($self->{alias} ne '');
...
#       }
    ...
    if ($self->{retries} == $self->{max_retries}) {
        die 'POE::Component::EasyDBI tried '.$self->{max_retries}
            .' times to create a Wheel and is giving up...';
    }

    my %prog = (
        'Program'       =>  \&POE::Component::EasyDBI::SubProcess::main,
        'ProgramArgs'   =>  [ $self->{opts} ],
    );

    if ($self->{opts}{alt_fork} && $^O ne 'MSWin32') {
        my $X = $self->{opts}{alt_fork} ne "1" ? $self->{opts}{alt_fork} : $^X;
        %prog = (
...
#    $kernel->sig_child( $self->{wheel_pid} )
#        if ( $self->{wheel_pid} );
    ...
    $self->{wheel_pid} = $self->{wheel}->PID();
    
    if ( $kernel->can( "sig_child" ) ) {
        $kernel->sig_child( $self->{wheel_pid} => 'sig_child' );
    } else {
    ...
    if (! defined $self->{wheel}) {
        ...
        if ($self->{opts}{alt_fork}) {
            $self->{wheel}->put($self->{opts});
        }
...
sub delete_query {
    ...
    if (defined($self->{queue}->[0])) {
        if ($self->{queue}->[0]->{id} eq $id) {
            # Query is still active, nothing we can do...
            return undef;
        } else {
            # Search through the rest of the queue and see what we get
            foreach my $count (@{ $self->{queue} }) {
                if ($self->{queue}->[$count]->{id} eq $id) {
                    # Found a match, delete it!
                    splice(@{ $self->{queue} }, $count, 1);
...
sub child_error {
    ...
    DEBUG && do {
        ...
        warn "POE::Component::EasyDBI got an $operation error $errnum from Subprocess: '$errstr' shutdown: $self->{shutdown}\n";
    };
...
#   DEBUG && do {
#       require Data::Dumper;
#       print Data::Dumper->Dump([$data,$self->{queue}[0]]);
#   };
    ...
    if ($data->{id} eq 'DBI') {
        ...
        if ($self->{no_connect_failures}) {
            my $qc = {};
            if (defined($self->{queue}->[0])) {
                $qc = $self->{queue}[0];
            }
            $qc->{error} = $data->{error};
            if (_ARRAY($self->{opts}{connect_error})) {
                $kernel->post(@{$self->{opts}{connect_error}}, $qc);
                delete $self->{queue}[0]->{error};
...
#           print "DBI error: $data->{error}, retrying\n";
    ...
    if ($data->{id} eq 'DBI-CONNECTED') {
        if (_ARRAY($self->{opts}{connected})) {
            my $query_copy = {};
            if (defined($self->{queue}->[0])) {
                $query_copy = { %{$self->{queue}[0]} };
            }
            $kernel->post(@{$self->{opts}{connected}}, $query_copy);
        }
        return;
    }
...
sub child_STDERR {
    ...
    return if ($_[OBJECT]->{opts}->{no_warnings} && !DEBUG);
...
=item C<combo>
    ...
    This is for running multiple queries in order.
            ...
            queries => [
                {
                 ...
                 },
                 {
                    insert => {
                        table => 'test',
                        hash => { id => 2, foo => 7891011, bar => 'blah' },
                    },
                 },
            ],
            event => 'combo_handler',
    ...
    The Success Event handler will get a hash in ARG0:
    {
        queries         =>  The original queries sent
        result          =>  Array of results from each query in order
    }
...
This retrieves the number of items in the queue.  If it returns 0 then there
are no queries being processed and easydbi is idle.

    my $num = $self->queue_length();
