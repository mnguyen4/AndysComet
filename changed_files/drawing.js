...
var cometd = dojox.cometd; //make cometd into an object so that it is easier to work with
var publish_flag = true; //a flag that will enable/disable publishing
(function(){
		...
		var whiteboard = {
			...
			receive: function(message) {
				if(message != null)
				{
					...
					var draw = message.data.drawing;
					var flag = message.data.action;

					console.info("flag = ", flag);

					if(from != this._username)
					{
						switch(flag)
						{
							case 1: // Handle drawing traffic
	...
	andes.drawing = {
		...
		add: function(/* Stencil */ item, /*Boolean*/ saveToServer, /*Boolean*/noConnect){
			...
			item.connect("onDelete", this, function(item){
				...
				this.remove(item);
				if(!item.mod){
					this.save({action:"delete-object", id:item.id}); // TODO: Need to remove from MasterMap as well
				}
		...
		handleServerActions: function(data){
			...
			dojo.forEach(data, function(obj, i){
				...
				if(obj.action =="new-object"){
					...
					if(items[obj.id]){
						console.log("In Delete Object call");
		...
		onSelected: function(stencil){
			...
			if (stencil.hasOwnProperty("combo")) // Combo (Rect/Eclipse)
			{
			...
			} else if (stencil.hasOwnProperty("customType")) // Line/Vector
			{
				if (stencil.customType == "equation")
				{
					item = items[stencil.id]; // Equation
				} else
				{
		...
		onDeselected: function(stencil, keepObject) {
